...
class WooThemes_Sensei_Certificates {
	...
	public function can_view_certificate( $certificate_id = 0 ) {
		...
		if ( true == (bool)$grant_access_user || ( false == (bool)$grant_access && true == (bool)$grant_access_user ) || current_user_can( 'manage_options' ) || current_user_can( 'site-admin' ) ) return true;
	...
	public function certificate_text( $pdf_certificate, $fpdf ) {
		...
		if ( 0 < intval( $certificate_id ) ) {
			...
			$lname = $student->last_name;
  			...
  			$nok_number = get_user_meta( $user_id, 'company', true );
        	$nnumber = $nok_number;
			...
			$certificate_place = $nnumber; // At {{course_place}}
			if ( isset( $this->certificate_template_fields['certificate_place']['text'] ) && '' != $this->certificate_template_fields['certificate_place']['text'] ) {
				...
				$certificate_place = str_replace( array( '{{learner}}', '{{course_title}}', '{{completion_date}}', '{{course_place}}'  ), array( $student_name, $course->post_title, $date, $nnumber ) , $certificate_place );
